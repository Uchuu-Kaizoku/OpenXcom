...
void BuildNewBaseState::resize(int &dX, int &dY)
{
	for (auto* surface : _surfaces)
	{
		...
		if (Options::oxceNewBaseHoverAreaInfo == 2 && (surface == _windowArea || surface == _txtArea)) // Bottom
		{
			surface->setY(surface->getY() + dY);
		}
		else if (surface != _window && surface != _btnCancel && surface != _txtTitle && surface != _windowArea && surface != _txtArea)
		{
